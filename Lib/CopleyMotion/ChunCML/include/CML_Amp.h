...
public:
   ...
   TPDO_Status(){ SetRefName( "TPDO_Status" ); ampPtr=0; }
   virtual ~TPDO_Status(){ KillRef(); }
...
*/
/***************************************************************************/
class TPDO_PvtBuffStat: public TPDO, Thread
{
   Semaphore sem;
   class Amp *ampPtr;                ///< Points to the amplifier to which this PDO belongs
...
public:
   /// Default constructor for this PDO
   TPDO_PvtBuffStat(){ SetRefName( "TPDO_PvtBuffStat" ); ampPtr=0; }
   virtual ~TPDO_PvtBuffStat(){ KillRef(); }
   const Error *Init( Amp &amp, uint16 slot );
   virtual void Received( void );
   void run( void );
...
public:
   /// Default constructor for this PDO
   RPDO_Pvt(){ SetRefName( "RPDO_Pvt" ); ampPtr = 0; }

   virtual ~RPDO_Pvt(){ KillRef(); }
...
{
   class Amp *ampPtr;                ///< Points to the amplifier to which this PDO belongs
   Pmap16 ctrl;                      ///< Used to map the CAN control word passed in the PDO

public:
   /// Default constructor for this PDO
   RPDO_Ctrl(){ SetRefName( "RPDO_Ctrl" ); ampPtr = 0; }

   virtual ~RPDO_Ctrl(){ KillRef(); }

   const Error *Init( Amp &amp, uint16 slot, uint16 id );

   const Error *Transmit( uint16 val );

private:
   /// Private copy constructor (not supported)
   RPDO_Ctrl( const RPDO_Ctrl& );

   /// Private assignment operator (not supported)
   RPDO_Ctrl& operator=( const RPDO_Ctrl& );
};
...
private:
   ...
   /// A receive PDO used to transmit control word values to the amp
   RPDO_Ctrl ctrlPdo;
