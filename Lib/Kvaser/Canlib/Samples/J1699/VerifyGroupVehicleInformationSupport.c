...
STATUS VerifyGroupVehicleInformationSupport(void)
{
	...
	{
		LogPrint("FAILURE: SID $9 group support request failed\n");
		return(FAIL);
	}
	...
	for (EcuIndex = 0; EcuIndex < gOBDNumEcus; EcuIndex++)
	{
		...
		{
			LogPrint("FAILURE: ECU %X  SID $9 group/individual support response size mismatch\n", GetEcuId(EcuIndex) );
			return(FAIL);
		}
		...
		for (IdIndex = 0; IdIndex < 8; IdIndex++)
		{
			for (IdBitIndex = 0; IdBitIndex < 4; IdBitIndex++)
			{
				...
				{
					LogPrint("FAILURE: ECU %X  SID $9 group/individual support response mismatch\n", GetEcuId(EcuIndex) );
					return(FAIL);
				}
			}
		}
	...
	// For mode $09, same number of controller must respond for group as well as individual support request!
	if ( EcuIndex != gOBDNumEcus )
	{
		LogPrint("FAILURE: SID $9 group support request failed.  Invalid number of controller responses.\n");
		return(FAIL);
	}
