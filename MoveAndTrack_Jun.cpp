...
int _tmain(int argc, _TCHAR* argv[])
{
   ...
   // Create an 'CanInterface' object used to access the low level CAN network.
	...
	//// ---------------------------------------------------------------- //
	//// JHa - base1 & base2 rotating together
	//double inc = 10.936;		double vel = 10.936/36.0;		acqFlag = true;
	...
	//double inc = 10.936;		double vel = 10.936/72.0;		acqFlag = true;

	//pos[4] = 1.1*inc;	// minus = CCW, plus = CW
	//err = link.MoveTo(pos,vel,10.0*vel,10.0*vel,100.0*vel);
	//showerr( err, "Moving linkage" );
	//err = link.WaitMoveDone( 1000 * 1000 );
	//showerr( err, "waiting on linkage done" );

	//pos[4] -= 1.2*inc;	// minus = CCW, plus = CW
	//err = link.MoveTo(pos,vel,10.0*vel,10.0*vel,100.0*vel);
	//showerr( err, "Moving linkage" );
	//err = link.WaitMoveDone( 1000 * 100 );
	//showerr( err, "waiting on linkage done" );
	...
	// ---------------------------------------------------------------- //
	// JHa - Jiggling motion. 
	//double inc = 10.936;		double vel = 10.936/36.0;		acqFlag = false;
	//
	//int nStoppingStep = 36;
	//double jigglingAngle = 50;	// degree
	//double jigglingAngle_inc = inc * jigglingAngle/360.0;	// motor inc
	//int nJigglingStep = 20;

	//for(int i = 0; i <= nStoppingStep; i++)
	//{
	//	double curPos = (double)i/(double)nStoppingStep * inc;
	...
	//	for(int j = 0; j <= nJigglingStep; j++)
	//	{
	//		double curJigglingPos = curPos + pow(-1.0,(double)j) * jigglingAngle_inc * (double)(nJigglingStep - j)/(double)nJigglingStep;

	//		pos[4] = curJigglingPos;
	//		err = link.MoveTo(pos,vel,10.0*vel,10.0*vel,100.0*vel);
	//		showerr( err, "Moving linkage" );
	//		err = link.WaitMoveDone( 1000 * 1000 );
	//		showerr( err, "waiting on linkage done" );
	...
	// ---------------------------------------------------------------- //
	// JHa - motion1 for video of Piecewise constant tubes
	//double inc = 10.936;		double vel = 10.936/36.0;		acqFlag = false;
	//
	//int nStoppingStep = 36;
	//double jigglingAngle = 50;	// degree
	...
	// ---------------------------------------------------------------- //
		...
		// ---------------------------------------------------------------- //
	// JHa - motion2 for video of Piecewise constant tubes
	//double inc = 10.936;		double vel = 10.936/36.0;		acqFlag = false;
	//
	//int nStoppingStep = 36;
	//double jigglingAngle = 50;	// degree
	...
	//// ---------------------------------------------------------------- //
	//// jha - 5 times of forward & backward rotations
	//double inc = 10.936;		double vel = 10.936/72.0;		acqflag = true;
	...
	//	err = link.moveto(pos,vel,10.0*vel,10.0*vel,100.0*vel);
	//	showerr( err, "moving linkage" );
	//	err = link.waitmovedone( 1000 * 500 );
	//	showerr( err, "waiting on linkage done" );
	//	
	//	::sleep(1000);
	//	
	...
	//// JHa - ???
	//double inc = 10.936;		double vel = 10.936/36.0;		acqFlag = true;
	//int nStep = 11;			double nStepDouble = (double)nStep;
	//int nOvershootStep = nStep/11;
	...
	//	{
	//		char cinVal;
	//		pos[3] = inc*1.1*(double)(j+1)/nStepDouble;
	//		err = link.MoveTo(pos,vel,10.0*vel,10.0*vel,100.0*vel);
	//		showerr( err, "Moving linkage" );
	//		err = link.WaitMoveDone( 1000 * 500 );
	//		showerr( err, "waiting on linkage done" );
	//		cout << "<<" <<pos[3] * 360.0 / inc << ">>" << endl;
	//		//std::getchar();
	//		cin >> cinVal;
	//	}

	//	// 1.1 => -0.1
	//	for(int j = 0; j< nStep + nOvershootStep ; j++)
	//	{
	//		char cinVal;
	//		pos[3] = inc*1.1*(double)(nStep-j-1)/nStepDouble;
	//		err = link.MoveTo(pos,vel,10.0*vel,10.0*vel,100.0*vel);
	//		showerr( err, "Moving linkage" );
	//		err = link.WaitMoveDone( 1000 * 500 );
	//		showerr( err, "waiting on linkage done" );
	...
	//			
	//	// -0.1 => 0
	//	for(int j = 0; j< nOvershootStep ; j++)
	//	{
	//		char cinVal;
	//		pos[3] = inc*1.1*(double)(-nOvershootStep + j + 1)/nStepDouble;
	//		err = link.MoveTo(pos,vel,10.0*vel,10.0*vel,100.0*vel);
	//		showerr( err, "Moving linkage" );
	//		err = link.WaitMoveDone( 1000 * 500 );
	//		showerr( err, "waiting on linkage done" );
	...
	//// JHa - (N/2) times of forward & backward rotations converging to 180deg
	//int N = 50;		double inc = 10.936/N;		double vel = 10.936/36.0;		acqFlag = true;
	//
	//for(int i=0; i<(N/2); i++)
	//{
	//	if(i < (N/4) )
	...
	//	pos[3] = 10.936 - (inc*i);
	//	//cout<<pos[3]/10.936*360.0<<endl;
	//	err = link.MoveTo(pos,vel,10.0*vel,10.0*vel,100.0*vel);		showerr( err, "Moving linkage" );
	//	err = link.WaitMoveDone( 1000 * 500 );						showerr( err, "waiting on linkage done" );
	//	
	//	::Sleep(1000);
	//	
	//	pos[3] = 0 + inc*(i+1);
